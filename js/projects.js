...
// generate pills for tech stack
const generateTechStackPills = (stackArray) => {
  let result = [];
  for (let stackElement in stackArray) {
    const listItem = `<a href="#">${stackArray[stackElement]}</a>`;
    result.push(listItem);
  }
  return `<div class="tags">${result.join(" ")}</div>`;
};

var id = 0;

const generateCards = (cardDetail, index) => {
  const { Name, Repo, Owner, profiles, Description, TechStack } = cardDetail;

  const projectCard = `<article onmouseover="setData(${index})" class="card">
      <header class="card-header">
        <p>${Owner}</p>
        <h2>${Name}</h2>
      </header>
      <p>${limitWords(Description, 40)}</p>
      <div class="card-author">

        <div class="author-name">
          <div class="author-name-prefix">SWOC</div>
          <a href="${Repo}">Github</a>
        </div>
      </div>
      ${generateTechStackPills(TechStack)}
    </article>`;
  return projectCard;
};
...
const injectCardsToPage = (project) => {
  let members = project.map((item, index) => {
...
//Team
document.querySelector(".project-list").innerHTML = injectCardsToPage(
  projectsData
);
...
const setData = index => {
  document.querySelector(".proj-info .proj-descrp .descrp p").innerHTML = `${projectsData[index].Description}`
  document.querySelector(".proj-info .proj-descrp .proj-project a").setAttribute("href", `${projectsData[index].Repo}`);
  document.querySelector(".proj-info .proj-descrp .proj-project h2").innerHTML = `${projectsData[index].Name}`
  document.querySelector(".proj-info .proj-author .author h2").innerHTML = `${projectsData[index].Owner}`
  authorProfileLink[0].setAttribute("href", `${projectsData[index].profiles[0].github}`);
  authorProfileLink[1].setAttribute("href", `${projectsData[index].profiles[0].linkedin}`);
  document.querySelector(".proj-info .tech-stack .stack-btns").innerHTML = `${generateStack(projectsData[index].TechStack)}`;
}
